   ...
   container('docker') {
    docker.image('jrgreggdevops/docker-awscli:1.0').inside {
     ...
     withCredentials([usernamePassword(
      ...
      sh 'aws ec2 describe-instances --region us-west-2 --filter Name=tag:Owner,Values=nameofowner --query Reservations[*].Instances[*].[InstanceId,ImageId,LaunchTime,Tags[`Owner`].Value] > auditResults.txt'
     }
